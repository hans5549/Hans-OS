...
async function generateAccessible(
  ...
  accessibleRoutes.forEach((route) => {
    if (root && !route.meta?.noBasicLayout) {
      ...
      // 如果你的專案已经跟进了本次修改，移除了所有自定义菜单首级的BasicLayout，可以将这段if程式碼删除
      if (route.children && route.children.length > 0) {
        delete route.component;
      }
      // 根據router name判断，如果路由已经存在，则不再添加
      if (names?.includes(route.name)) {
        // 找到已存在的路由索引并更新，不更新会造成切换使用者时，一级目录未更新，homePath 在二级目录导致的404問題
...
async function generateRoutes(
   ...
   * 2. 将懒載入的元件名称修改为目前路由的名称（如果启用了keep-alive的话）
   */
  resultRoutes = mapTree(resultRoutes, (route) => {
    ...
    // 如果有redirect或者沒有子路由，则直接返回
    if (route.redirect || !route.children || route.children.length === 0) {
      return route;
    }
    const firstChild = route.children[0];

    // 如果子路由不是以/开头，则直接返回,这种情況需要计算全部父级的path才能得出正确的path，這裡不做处理
