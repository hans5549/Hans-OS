...
2. 当前前端以 `web-antd`（Ant Design Vue）为主，并保留 `docs` 与 `playground` 作为文档和演示入口。
...
`web-antd` 在 `src/adapter/` 下提供适配器，将 `@vben/common-ui` 的通用 form/modal/drawer 组件桥接到 Ant Design Vue 组件。
...
当前仓库不再内置 Nitro mock 应用。前端本地开发应通过 `VITE_GLOB_API_URL` 直接指向现有后端或外部 mock 服务。
