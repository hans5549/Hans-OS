...
public class BankTransactionServiceTests : IDisposable
{
    ...
    [Fact]
    public async Task BatchUpdateDepartment_WithLinkedActivityAndDifferentDepartment_ThrowsError()
    {
        ...
        var act = () => _sut.BatchUpdateDepartmentAsync(request);

        await act.Should().ThrowAsync<ArgumentException>()
            .WithMessage("*不可改到其他部門*");
    }
