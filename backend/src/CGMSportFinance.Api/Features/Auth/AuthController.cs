...
using CGMSportFinance.Api.Features.Auth.Contracts;
...
{
    /// <summary>
    /// Sign in with username and password, then issue an access token and refresh token cookie.
    /// </summary>
    [AllowAnonymous]
    [HttpPost("login")]
    ...
    public async Task<ActionResult<ApiEnvelope<AuthTokenResponse>>> Login([FromBody] LoginRequest request)
    {
        ...
        return Ok(ApiEnvelope<AuthTokenResponse>.Success(new AuthTokenResponse(accessToken.AccessToken, accessToken.ExpiresIn)));
    }

    /// <summary>
    /// Rotate the HttpOnly refresh token cookie and return a new access token.
    /// </summary>
    [AllowAnonymous]
    [HttpPost("refresh")]
    [ProducesResponseType(typeof(ApiEnvelope<AuthTokenResponse>), StatusCodes.Status200OK)]
    ...
    /// <summary>
    /// Revoke the current refresh token cookie and clear browser state.
    /// </summary>
    [AllowAnonymous]
    [HttpPost("logout")]
    ...
    public async Task<ActionResult<ApiEnvelope<object?>>> Logout()
    {
        ...
        {
            var tokenHash = tokenService.HashRefreshToken(rawToken);
            var existingToken = await dbContext.RefreshTokens.SingleOrDefaultAsync(token => token.TokenHash == tokenHash);
    ...
    /// <summary>
    /// Return button-level permission codes for the current user.
    /// </summary>
    [Authorize]
    [HttpGet("codes")]
    ...
    {
        var roleNames = User.FindAll(System.Security.Claims.ClaimTypes.Role).Select(claim => claim.Value).Distinct().ToArray();
    ...
    private ActionResult<ApiEnvelope<AuthTokenResponse>> InvalidCredentials()
    {
        return StatusCode(
            ...
            Problem(title: "Forbidden", detail: "Username or password is incorrect.", statusCode: StatusCodes.Status403Forbidden));
    }
